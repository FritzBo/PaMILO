...
OnlineVertexEnumeratorCDD::OnlineVertexEnumeratorCDD(Point &initial_value, unsigned int dimension,
                                                     double epsilon)
    ...
    , h_representation_(nullptr)
{
    ...
    for (unsigned int j = 0; j < dimension_ - 1; ++j)
        dd_set_d(h_representation_->matrix[dimension_][j + 1],
                 initial_value[j] - initial_value[dimension_ - 1]);
