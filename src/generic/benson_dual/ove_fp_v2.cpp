...
GraphlessOVE::GraphlessOVE(const Point &initial_value, unsigned dimension, double epsilon)
...
void GraphlessOVE::add_hyperplane(Point &vertex, Point &normal, double rhs)
{
    ...
    // candidate pair
    for (auto cut_off_it = cut_off_points.begin(); cut_off_it != cut_off_points.end(); ++cut_off_it)
    {
        GraphlessPoint &cut_off_point = ***cut_off_it;

        assert(cut_off_point.active_inequalities_.size() >= dimension_);

        for (auto inside_it = inside_points.begin(); inside_it != inside_points.end(); ++inside_it)
        {
            ...
            if (check_adjacent(cut_off_point, inside_point))
            {
                ...
                new_points.push_back(
                    add_cut_point(cut_off_point, inside_point, projective_hyperplane));
            }
        }
    }
...
bool GraphlessOVE::check_adjacent(GraphlessPoint &p1, const GraphlessPoint &p2)
{
#ifndef NDEBUG
    ...
    if (dimension_ <= 2)
    {
        ...
        set_intersection(p1.active_inequalities_.cbegin(), p1.active_inequalities_.cend(),
                         p2.active_inequalities_.cbegin(), p2.active_inequalities_.cend(),
    ...
    else
    {
        ...
        unsigned common_count = std::count(common_vertices.cbegin(), common_vertices.cend(), true);
#ifndef NDEBUG
        if (debug_output)
        {
...
auto GraphlessOVE::add_cut_point(const GraphlessPoint &outside_point, GraphlessPoint &inside_point,
                                 const Point &inequality) -> GraphlessPoint *
{
    ...
    set_intersection(
        outside_point.active_inequalities_.cbegin(), outside_point.active_inequalities_.cend(),
        inside_point.active_inequalities_.cbegin(), inside_point.active_inequalities_.cend(),
