 ...
 * last checkin:
 *   $Author: gutwenger $
 *   $Date: 2013-07-02 09:51:50 +0200 (Di, 02 Jul 2013) $
 ***************************************************************/
...
#ifndef MCO_BINARY_HEAP2_H
#define MCO_BINARY_HEAP2_H
...
class BinaryHeap2 : public ogdf::HeapBase<key, HeapObject>
{
public:
        //! Creates a binary heap.
        BinaryHeap2(int startSize = 128,
                    std::function<bool(key const &, key const &)> compare = [](key const &k1, key const &k2) { return k1 < k2; });
        ...
        //! Increases priority of an object that is addressed by \a index.
        // use updated m_foreign position index to address entry for increasekey
        virtual void increaseKey(int index, key priority);
        //TODO: version mit Aenderungswert statt absolutem Wert
        ...
        {
                OGDF_ASSERT( (index > 0) && (index <= ogdf::HeapBase<key,HeapObject>::m_size) );
        ...
        int size() const { return ogdf::HeapBase<key,HeapObject>::m_size; }

        //! Returns true iff the heap is empty.
        int empty() const { return ogdf::HeapBase<key,HeapObject>::empty(); }
...
protected:
        ...
        {
                OGDF_ASSERT(num>0);
                return (leftChildIndex(num) <= ogdf::HeapBase<key,HeapObject>::m_size);
        }
        ...
        {
                OGDF_ASSERT(num>0);
                return (rightChildIndex(num) <= ogdf::HeapBase<key,HeapObject>::m_size);
        }
...
private:
        ...
        // function that defines key order; returns true if first arg is less than second arg
        std::function<bool(key const &, key const &)> m_compare;
...
template <class key, class HeapObject>
BinaryHeap2<key, HeapObject>::BinaryHeap2(int startSize, std::function<bool(key const &, key const &)> compare)
...
void BinaryHeap2<key, HeapObject>::init(int initSize)
{
        ...
        ogdf::HeapBase<key,HeapObject>::m_size = 0;
}
...
void BinaryHeap2<key, HeapObject>::siftUp(int pos)
{
        OGDF_ASSERT( (pos > 0) && (pos <= ogdf::HeapBase<key,HeapObject>::m_size) )
                ...
                while ( (parentIndex(run) >= 1) &&
                        (m_compare(tempEntry.m_priority, m_heapArray[parentIndex(run)].m_priority)))
                {
...
void BinaryHeap2<key, HeapObject>::siftDown(int pos)
{
        ...
        if (pos >= int(ogdf::HeapBase<key,HeapObject>::m_size/2)+1)
        {
        ...
        if (hasLeft(pos) && (m_compare(getPriority(leftChildIndex(pos)), sPrio)) )
        {
                sIndex = leftChildIndex(pos);
                sPrio = getPriority(leftChildIndex(pos));
        }//if left child smaller
        if (hasRight(pos) && (m_compare(getPriority(rightChildIndex(pos)), sPrio)) )
        {
...
void BinaryHeap2<key, HeapObject>::makeHeap()
{
        ...
        for (int i=ogdf::HeapBase<key,HeapObject>::m_size/2; i > 0; i--)
                siftDown(i);
}//makeheap
...
//         if (m_compare(he.m_priority, priority)) OGDF_THROW_PARAM(AlgorithmFailureException, afcIllegalParameter);
...
template <class key, class HeapObject>
void BinaryHeap2<key, HeapObject>::increaseKey(int index, key priority)
{
        HeapEntry& he = m_heapArray[index];

        //check if error value
//         if (m_compare(priority, he.m_priority)) OGDF_THROW_PARAM(AlgorithmFailureException, afcIllegalParameter);
...
{
        OGDF_ASSERT((!ogdf::HeapBase<key,HeapObject>::empty()));

        HeapEntry tempEntry = m_heapArray[1]; //save minimum object

        ogdf::HeapBase<key,HeapObject>::m_size--;

        if (ogdf::HeapBase<key,HeapObject>::m_size > 0)
        {
                m_heapArray[1] = m_heapArray[ogdf::HeapBase<key,HeapObject>::m_size+1]; //old last leaf

                //check if reallocation is possible
                if ((ogdf::HeapBase<key,HeapObject>::m_size < (m_arraySize/3)) && (m_arraySize > 2*m_startSize-1))
                {
                        HeapEntry* tempHeap = new HeapEntry[lowerArrayBound(m_arraySize)];
                        for (int i = 1; i <= ogdf::HeapBase<key,HeapObject>::m_size ; i++)
...
{
        OGDF_ASSERT((ogdf::HeapBase<key,HeapObject>::m_size) < m_arraySize);
        ogdf::HeapBase<key,HeapObject>::m_size++;
        //check if the array size has to be adjusted
        if (ogdf::HeapBase<key,HeapObject>::m_size == m_arraySize)
        {
        ...
        m_heapArray[ogdf::HeapBase<key,HeapObject>::m_size] = HeapEntry(priority, ho, ogdf::HeapBase<key,HeapObject>::m_size, keyUpdate);

        siftUp(ogdf::HeapBase<key,HeapObject>::m_size);
...
const BinaryHeap2<key, HeapObject>& BinaryHeap2<key, HeapObject>::operator=(const BinaryHeap2<key, HeapObject>& rhs)
{
        if (this != &rhs)
        {
                ...
                ogdf::HeapBase<key,HeapObject>::m_size = rhs.m_size;
