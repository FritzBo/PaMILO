...
void PrimalBensonMolpSolver<OnlineVertexEnumerator>::Solve() {
	...
	while(vertex_container.has_next()) {
		...
		// Finding tight inequalities for this solution
//		for(unsigned int i = 0; i < model_.constraints(); ++i) {
//
//			Point inequality(&model_.A()[i * model_.variables()], model_.variables());
//
//			cout << inequality << endl;
//
//			cout << inequality * (*x) - model_.b()[i] << endl;
//
//			if(abs(inequality * (*x) - model_.b()[i]) < epsilon_) {
//				Point image(model_.objectives() + 1);
//
//				unsigned int l1_norm = 0;
//				for(unsigned int j = 0; j < model_.objectives(); ++j) {
//					for(unsigned int k = 0; k < model_.variables(); ++k)
//						image[j] += inequality[k] * model_.C()[j * model_.variables() + k];
//					l1_norm += abs(image[j]);
//				}
//
//				for(unsigned int j = 0; j < model_.objectives(); ++j)
//					image[dimension] += (*y_prime)[j] * image[j];
//
//				for(unsigned int j = 0; j < model_.objectives() + 1; ++j)
//					image[j] /= image[dimension];
//
//				cout << "objective inequality: " << image << endl;
//			}
//		}
		// Finished finding tight inequalities
